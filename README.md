...
Then, the now 'spring' velocity of the card is mapped to its rotation; The faster the card moves, the more it will rotate in response. This is done using the useTransform( ... ) function. Any velocity values between -6000 and 6000 are turned into -60 and 60 (in this case degress of rotation) respectively. These values are assigned to the variable _rotate_, which is normally an automatically created MotionValue, then it is to the motion.div element via the Style attribute.
